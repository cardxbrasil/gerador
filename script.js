...
- **Público-Alvo (Específico):** Defina o espectador ideal para esta investigação (interesses intelectuais, demografia, psicografia).
...
**AÇÃO FINAL:** Mergulhe profundamente no relatório. Extraia os fatos mais relevantes, identifique as conexões não óbvias e construa 6 propostas documentais que mantenham o rigor dos fatos enquanto criam narrativas irresistíveis. Responda APENAS no formato markdown especificado.`,

    'inspiracional': `Você é uma API DE ELITE em CRIAÇÃO DE CONTEÚDO NARRATIVO INSPIRADOR E TRANSFORMADOR. Sua função é atuar como um ARQUITETO DE JORNADAS EMOCIONAIS, mestre na arte de transformar fatos aparentemente ordinários em narrativas que tocam a alma humana e inspiram ação.

**IDENTIDADE E ESPECIALIZAÇÃO (A REGRA MAIS IMPORTANTE):**
Você não é apenas um contador de histórias inspiradoras, você é um ALQUIMISTA EMOCIONAL. Sua especialidade é identificar o ouro da experiência humana oculto nos dados brutos e transformá-lo em narrativas que não apenas emocionam, mas capacitam o espectador.
...
**MANUAL DE CRIAÇÃO DETALHADO (SIGA EXATAMENTE PARA CADA IDEIA):**
-   **Título (Emocional e Transformador):** Crie um título que funcione como um farol de esperança e prometa uma jornada de transformação significativa.
...
-   **Descrição (Narrativa Rica e Emocional):** Uma sinopse completa de **pelo menos 5 frases** que estabeleça o ponto de partida emocional, introduza o obstáculo, descreva a jornada da transformação e termine com a lição universal.
...
**AÇÃO FINAL:** Encontre as histórias de resiliência e transforme fatos em narrativas que capacitem o espectador a ver suas lutas sob uma nova luz. Responda APENAS no formato markdown especificado.`,
...
**TAREFA CRÍTICA:** Sua missão é gerar 6 ideias de vídeos que transcendam conexões superficiais, criando pontes teológicas profundas entre os DADOS do relatório e as Escrituras.
**IDIOMA OBRIGATÓRIO:** Todas as respostas DEVEM estar em __LANGUAGE_NAME__.

**MANUAL DE CRIAÇÃO DETALHADO (SIGA EXATAMENTE PARA CADA IDEIA):**
-   **Título (Cativante e Teológico):** Deve prometer uma revelação transformadora que conecte a descoberta com uma verdade bíblica profunda.
-   **Ângulo (O Enigma Central):** Uma frase complexa que apresente uma conexão inovadora entre um FATO do relatório, uma PASSAGEM BÍBLICA e uma IMPLICAÇÃO TEOLÓGICA.
-   **Público-Alvo (Específico):** Descreva com precisão o nicho de espectador (ex: "Pastores", "Estudantes de teologia").
-   **Potencial Viral (Nota de Revelação):** Uma nota de 1 a 10 para o potencial da ideia de gerar DEBATE TEOLÓGICO.
-   **Profundidade Teológica:** Uma nota de 1 a 10 que avalia a profundidade e originalidade da conexão teológica.
-   **Fundamentação Bíblica:** Liste 3-5 referências bíblicas-chave.
-   **Descrição (Investigativa e Rica):** Escreva uma sinopse de **pelo menos 7 frases** que construa uma narrativa estimulante, apresentando o mistério, a descoberta, as implicações e possíveis objeções.
...
**AÇÃO FINAL:** Como Coletivo Hermenêutico, desvende conexões teológicas ousadas e gere as 6 ideias. Responda APENAS no formato markdown especificado.`,
...
-   **Descrição (Irresistível):** Uma sinopse de **pelo menos 5 frases** que inclua um gancho, 2-3 fatos impactantes e um "momento uau".
...
// ... (após a função getIdeasPrompt e sua vírgula)

getParserPrompt: (genre, creativeText) => {
    const parsers = {
        'documentario': `**MAPEAMENTO DE CHAVES:** "Título" -> "title", "Ângulo" -> "angle", "Público-Alvo" -> "targetAudience", "Potencial Viral" -> "viralityScore", "Descrição" -> "videoDescription", "Abordagem Investigativa" -> "investigativeApproach".`,
        'inspiracional': `**MAPEAMENTO DE CHAVES:** "Título" -> "title", "Ângulo" -> "angle", "Público-Alvo" -> "targetAudience", "Potencial Viral" -> "viralityScore", "Descrição" -> "videoDescription", "Núcleo Emocional" -> "emotionalCore".`,
        'scifi': `**MAPEAMENTO DE CHAVES:** "Título" -> "title", "Ângulo" -> "angle", "Público-Alvo" -> "targetAudience", "Potencial Viral" -> "viralityScore", "Descrição" -> "videoDescription", "Dilema Central" -> "coreDilemma".`,
        'terror': `**MAPEAMENTO DE CHAVES:** "Título" -> "title", "Ângulo" -> "angle", "Público-Alvo" -> "targetAudience", "Potencial Viral" -> "viralityScore", "Descrição" -> "videoDescription", "Mecanismo de Terror" -> "horrorMechanism".`,
        'enigmas': `**MAPEAMENTO DE CHAVES:** "Título" -> "title", "Ângulo" -> "angle", "Público-Alvo" -> "targetAudience", "Potencial Viral" -> "viralityScore", "Profundidade Teológica" -> "theologicalDepth", "Fundamentação Bíblica" -> "scripturalFoundation", "Descrição" -> "videoDescription", "Perguntas para Diálogo" -> "discussionQuestions".`,
        'geral': `**MAPEAMENTO DE CHAVES:** "Título" -> "title", "Ângulo" -> "angle", "Público-Alvo" -> "targetAudience", "Potencial Viral" -> "viralityScore", "Descrição" -> "videoDescription", "Gatilhos de Compartilhamento" -> "shareTriggers".`
    };
    
    const parserInstructions = parsers[genre] || parsers['geral'];
...
2.  ${parserInstructions}
3.  **CONVERSÃO DE ARRAYS:** Os campos "Fundamentação Bíblica", "Perguntas para Diálogo" e "Gatilhos de Compartilhamento" DEVEM ser convertidos em arrays de strings.
...
const generateIdeasFromReport = async (button) => {
    ...
    try {
        ...
        const parserPrompt = PromptManager.getParserPrompt(genre, creativeText);
